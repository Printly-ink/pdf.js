...
export function usePdfViewer({
  ...
  React.useEffect(() => {
    ...
    async function loadScript() {
      try {
        const pdfjsLib = await import(pdfjsLibSrc);
...
export type ToolbarHorizontalGroupProps = {
  as?: "div" | "span",
...
export function ToolbarHorizontalGroup({
  ...
  return (
    React.createElement(
      as,
      { className: `toolbarHorizontalGroup ${className ? className: ''}`, id, role, "aria-live": ariaLive },
      children
    )
  )
}
...
export function HorizontalToolbarSeparator({ id, clasName }: { id?: string, clasName?: string }) {
  return <div id={id} className={`horizontalToolbarSeparator ${clasName ? clasName : ''}`} />
}

export function VerticalToolbarSeparator({ id, clasName }: { id?: string, clasName?: string }) {
  return <div id={id} className={`verticalToolbarSeparator ${clasName ? clasName : ''}`} />
}
